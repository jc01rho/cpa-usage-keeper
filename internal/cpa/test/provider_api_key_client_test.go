...
func TestProviderAPIKeyClientsUseDedicatedEndpointsAndDecodePayloads(t *testing.T) {
	...
	cases := []providerEndpointCase{
		...
		// CommandCode 是本地专属 management endpoint，保持标准 key DTO。
		{name: "commandcode", path: "/v0/management/commandcode-api-key", directBody: `[{"api-key":"commandcode-key","prefix":"commandcode-prefix","base-url":"https://commandcode.example/v1","name":"CommandCode","auth-index":"commandcode-auth"}]`, wrappedBody: `{"commandcode-api-key":[{"api-key":"commandcode-key","prefix":"commandcode-prefix","base-url":"https://commandcode.example/v1","name":"CommandCode","auth-index":"commandcode-auth"}]}`},
...
func fetchProviderEndpoint(ctx context.Context, client *cpa.Client, source string) (providerEndpointResult, error) {
	...
	case "vertex":
		...
		return providerEndpointResult{statusCode: result.StatusCode, body: result.Body, keys: result.Payload}, err
	case "commandcode":
		// CommandCode 使用标准 key result。
		result, err := client.FetchCommandCodeAPIKeys(ctx)
		// 返回 CommandCode HTTP 元数据与 payload。
		return providerEndpointResult{statusCode: result.StatusCode, body: result.Body, keys: result.Payload}, err
	case "openai":
