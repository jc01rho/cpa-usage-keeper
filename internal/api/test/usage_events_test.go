...
func TestUsageEventsExportCSVReturnsFilteredRowsWithoutPagination(t *testing.T) {
	...
	if !regexp.MustCompile(`(?m)^id,instance_id,timestamp,api_key,cpa_api_key_id,source,source_type,auth_index,is_identity_deleted,model,model_alias,response_model,reasoning_effort,`).MatchString(body) {
		t.Fatalf("expected instance_id and response_model in csv header, got %s", body)
	}
