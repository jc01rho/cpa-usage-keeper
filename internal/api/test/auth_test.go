...
//go:linkname filterManagedSessionRecordsByAPIKeys cpa-usage-keeper/internal/api.filterManagedSessionRecordsByAPIKeys
func filterManagedSessionRecordsByAPIKeys(records []auth.SessionRecord, apiKeysByID map[int64]entities.CPAAPIKey) []auth.SessionRecord
...
// TestFilterManagedSessionRecordsByAPIKeysKeepsAdminsAndScopedViewers preserves the
// fork's instance-scoped managed-session filtering: admins always stay, API key
// viewers survive only when their key id is present in the active key set.
