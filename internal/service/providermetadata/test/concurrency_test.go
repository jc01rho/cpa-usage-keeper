...
// registrySourceOrder 复用用户指定的稳定九来源顺序。
var registrySourceOrder = []string{"codex", "xai", "gemini", "gemini-interactions", "claude", "vertex", "commandcode", "meta", "openai"}
...
// FetchCommandCodeAPIKeys 在 commandcode gate 释放后返回一条标准 Credential 输入。
func (f *gatedProviderFetcher) FetchCommandCodeAPIKeys(ctx context.Context) (*response.ProviderKeyConfigResult, error) {
	// 等待测试释放 CommandCode 或取消 context。
	if err := f.wait(ctx, "commandcode"); err != nil {
		return nil, err
	}
	// 返回带唯一 auth-index 的成功 CommandCode payload。
	return standardSuccessResult("commandcode"), nil
}
