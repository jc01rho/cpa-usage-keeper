...
func applyUsageEventToComparisonOnly(comparisons *dto.UsageOverviewComparisonsRecord, event entities.UsageEvent, resolver pricing.Resolver, identityLookup analysisIdentityLookup) {
	...
	applyUsageOverviewIdentityComparison(comparisons, identityLookup, event.InstanceID, event.AuthIndex, row)
}
...
func applyUsageOverviewIdentityComparison(comparisons *dto.UsageOverviewComparisonsRecord, identityLookup analysisIdentityLookup, instanceID, authIndex string, row dto.UsageComparisonItemRecord) {
	if identity, ok := identityLookup.find(entities.UsageIdentityAuthTypeAuthFile, instanceID, strings.TrimSpace(authIndex)); ok {
		row.Label = identity.label
		addUsageOverviewComparison(comparisons.AuthFiles, identity.identity, row)
	}
	if identity, ok := identityLookup.find(entities.UsageIdentityAuthTypeAIProvider, instanceID, strings.TrimSpace(authIndex)); ok {
		row.Label = identity.label
...
func loadAndApplyUsageOverviewStats(overview *dto.UsageOverviewRecord, db *gorm.DB, filter dto.UsageQueryFilter, start, end time.Time, grain string, bucketByDay bool, resolver pricing.Resolver) error {
	if filter.ComparisonOnly {
		...
		for _, row := range rows {
			...
			applyUsageOverviewIdentityComparison(overview.Comparisons, identityLookup, filter.InstanceID, row.AuthIndex, comparison)
		}
		return nil
	}
