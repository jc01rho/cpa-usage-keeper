...
func TestCPAAPIKeyListIncludesInstanceIdentityAndScopesCollidingKeys(t *testing.T) {
	...
	router := keeperapi.NewRouter(nil, nil, nil, nil, keeperapi.AuthConfig{}, nil, "", keeperapi.OptionalProviders{CPAInstances: instanceService, CPAAPIKeys: service.NewCPAAPIKeyService(db)})
...
func TestCPAAPIKeyOptionsReturnActiveLabels(t *testing.T) {
	...
	if len(parsed.Options) != 1 || parsed.Options[0].ID != "1" || parsed.Options[0].InstanceID == "" || parsed.Options[0].DisplayKey != "sk-*********123456" || parsed.Options[0].Label != "Primary Key" {
...
func TestUpdateCPAAPIKeyAliasRejectsAnotherInstanceKey(t *testing.T) {
	...
	router := keeperapi.NewRouter(nil, statusStub{}, nil, nil, keeperapi.AuthConfig{}, nil, "", keeperapi.OptionalProviders{CPAAPIKeys: service.NewCPAAPIKeyService(db)})
