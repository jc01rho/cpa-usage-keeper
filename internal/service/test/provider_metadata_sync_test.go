...
func TestProviderMetadataSyncPreservesSourceFields(t *testing.T) {
	...
	// 九个 provider endpoint 每轮都必须恰好调用一次。
	for _, source := range []string{"codex", "xai", "gemini", "gemini-interactions", "claude", "vertex", "commandcode", "meta", "openai"} {
...
func TestProviderMetadataSyncCompletionOrderDoesNotChangeDatabase(t *testing.T) {
	...
	registryOrder := []string{"codex", "xai", "gemini", "gemini-interactions", "claude", "vertex", "commandcode", "meta", "openai"}
	...
	orders := []struct {
		...
		// 正序与 registry 完全一致。
		{name: "forward", completionOrder: []string{"codex", "xai", "gemini", "gemini-interactions", "claude", "vertex", "commandcode", "meta", "openai"}},
		// 逆序证明 fold 不依赖最快返回来源。
		{name: "reverse", completionOrder: []string{"openai", "meta", "commandcode", "vertex", "claude", "gemini-interactions", "gemini", "xai", "codex"}},
		// 混合顺序覆盖相邻来源交错完成。
		{name: "mixed", completionOrder: []string{"gemini", "openai", "codex", "claude", "xai", "vertex", "commandcode", "meta", "gemini-interactions"}},
	}
