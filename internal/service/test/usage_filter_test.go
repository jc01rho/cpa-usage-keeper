...
func TestUsageServiceResolvesAPIKeyIDForUsageQueries(t *testing.T) {
	db := openUsageServiceTestDatabase(t)
	targetID := seedUsageFilterAPIKeys(t, db)
	var otherKey entities.CPAAPIKey
	if err := db.Where("api_key = ?", "sk-other-key").First(&otherKey).Error; err != nil {
		t.Fatalf("load other seeded API key id: %v", err)
	}
	otherID := strconv.FormatInt(otherKey.ID, 10)
	if _, _, err := repository.InsertUsageEvents(db, []entities.UsageEvent{
