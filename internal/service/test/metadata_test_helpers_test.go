...
// metadataTestFetcher 是九来源并发安全的函数式测试 fetcher，默认所有 endpoint 成功返回空列表。
...
func newMetadataTestFetcher() *metadataTestFetcher {
	...
	// 九个标准 endpoint 都显式设置为 200 空 payload。
	for _, source := range []string{"codex", "xai", "gemini", "gemini-interactions", "claude", "vertex", "commandcode", "meta"} {
...
// FetchCommandCodeAPIKeys 读取 CommandCode 测试 endpoint。
func (f *metadataTestFetcher) FetchCommandCodeAPIKeys(ctx context.Context) (*response.ProviderKeyConfigResult, error) {
	// CommandCode 使用标准 provider 分派。
	return f.fetchStandardProvider(ctx, "commandcode")
}
