...
func TestLocalRankingProfileRouteUpdatesAliasAndAvatar(t *testing.T) {
	...
	response := rankingResponse(localRankingRouter(provider), http.MethodPatch, "/api/v1/ranking/local/profiles/42", `{"key_alias":"  Primary  ","avatar_id":17}`)
...
func TestLocalRankingRouteValidatesSelectionAndDisablesCaching(t *testing.T) {
	...
	response := rankingResponse(router, http.MethodGet, "/api/v1/ranking/local/leaderboards?period=today&metric=overall", "")
	...
	filtered := rankingResponse(router, http.MethodGet, "/api/v1/ranking/local/leaderboards?period=today&metric=overall&instance_id=all", "")
