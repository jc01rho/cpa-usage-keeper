...
func TestUsageAnalysisReturnsAggregatedRows(t *testing.T) {
	...
	if !strings.Contains(body, `"key":"`+helper.RedactSensitiveValue("sk-provider123456")+`"`) || !strings.Contains(body, `"label":"`+helper.RedactSensitiveValue("sk-provider123456")+`"`) {
		t.Fatalf("expected masked api key composition in response body: %s", body)
	}
